...
#if defined(NDEBUG)
# error "Dougcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd765e53c5dd4ef627b57516e71a21a5cf9f33964776d331f7c98cf9b011a0f34");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dougcoin: starting difficulty is 1 / 2^12
...
{
    // Dougcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!IsCoinBase())
        return 0;
    return max(0, (COINBASE_MATURITY+12) - GetDepthInMainChain());
}
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 134400); // Dougcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 24 * 60 * 60; // Dougcoin: 5 days
static const int64 nTargetSpacing = 3 * 60; // Dougcoin: 3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dougcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dougcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xff1d94c81c0576558b753239566ec5b53011696e91a9a8cfc56a9d05fc81119f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "7/5/2021 - Azerbaijan mud volcano triggers huge blast in Caspian oil and gas fields";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 8 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d2ea6a14f5fbaf9750a4bc4472459c42cf71761a1a402b37d21e5392b9f5a8436bbf62540cf49f8f17bca86e00ee70ce9966028e12f835e206a042f621f5873c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1625516437;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084968494;

        if (fTestNet)
        {
            block.nTime    = 1625516422;
            block.nNonce   = 387110309;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1e4d4c6c623ff8e554f08eb13d6069b9c567921bf09edf064b10a763929d8ba7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc6, 0xb3, 0xdf }; // Dougcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DougcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DougcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DougcoinMiner(CWallet *pwallet)
{
    printf("DougcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dougcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DougcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DougcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DougcoinMiner, pwallet));
}
